    ...
    <div className=" [@media(max-width:660px)]:w-full">
      <div className="w-full flex flex-col gap-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 bg-white-4 rounded-[12px] overflow-hidden p-1 h-11">
          {
            ["Game data", "Betting data"].map((item, index) => <button className={cn("text-[14px] font-bold rounded-[8px] overflow-hidden text-center", selectedCurrencyType === item ? "bg-white-13 text-gallery" : "text-casper")} onClick={() => handleCurrencyTypeClick(item)}>{item}</button>)
          }
        </div>
            ...
            <>
              ...
              <div className="rounded-t-[8px] overflow-hidden">
                <div className="grid gap-4 grid-cols-[auto_70px_70px_70px] p-4 bg-mirage  items-center text-casper font-bold text-[12px]">
                  <span>Game</span>
                  <span className="text-center">Number of Bets</span>
                  <span className="text-center">Bet amount</span>
                  <span className="text-center">Win or lose</span>
                </div>
                {
                  games.map((game, index) => <div key={index} className={cn("grid h-12 gap-4 grid-cols-[auto_70px_70px_70px] px-4 py-2 items-center font-bold text-[12px]", index % 2 === 0 ? "bg-white-4": "bg-white-8")}>
                    ...
                    <div className="text-center text-white">{game.numberBet}</div>
                    <div className="text-center text-white">{game.betAmount}</div>
                    <div className="text-center text-dodger-blue">{game.win}</div>
                  </div>)
                }
              </div>
            </>
