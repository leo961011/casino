    ...
    <div className=" [@media(max-width:660px)]:w-full flex flex-col gap-4">
      ...
      <div>
        <div className="grid h-[56px] border-b border-ebony-clay items-center grid-cols-[auto_60px_60px_60px] gap-4 py-2 px-4 rounded-t-[8px] bg-mirage text-blue-bayoux text-[12px] font-bold">
          ...
          <span className="word-break text-center">Bet amount</span>
          <span className="word-break text-center">Win or lose</span>
        </div>
        <div className="flex flex-col gap-px rounded-[8px] overflow-hidden">
          {
            history.map((item, index) => 
              <div key={index}  onClick={() => { setModalIndex(index); setModalOpen(true)}} className={cn("grid items-center grid-cols-[auto_60px_60px_60px] gap-4 py-2 px-4 font-bold", index % 2 === 0 ? "bg-white-4": "bg-white-8")}>
                ...
                <span className="text-[12px] text-casper text-center">{item.betamount}</span>
        ...
        <div className="text-center text-casper text-[12px]">No more</div>
    </div>
  );
};
