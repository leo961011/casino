      ...
      <ModalContainer isOpen={isModalOpen} onClose={toggleModal} title="Activation address" className="lg:w-[30%]">
          <div className="flex flex-col gap-2">
            <div className="p-1 rounded-[0.75rem] bg-white-4 gap-4 grid grid-cols-2">
              {
                tabs.map(tab => <button onClick={() => setSelectedTab(tab.id)} className={cn("h-9 font-bold text-[0.875rem] flex justify-center items-center rounded-[0.5rem]", tab.id === selectedTab ? "bg-white-13 text-white": "text-casper")}>{tab.desc}</button>)
              }
            </div>
            {
              selectedTab === "add" ? 
            ...
            <h2 className="text-[0.875rem] font-bold text-white">Activated address: <span className="text-dodger-blue">0</span></h2>
                {
                  ...
                  <div className="grid font-bold text-[12px] bg-white-4   grid-cols-[20%_40%_40%] items-center h-[48px] text-white">
                      ...
                      <span className="text-center">TGFf7****ePvThd</span>
                      <span className="text-center font-medium">To be activated</span>
                  </div>
                </div>:
                <div className="h-px bg-mirage-4 w-full" />
                }
                <button onClick={toggleAddress} className="mx-auto">
