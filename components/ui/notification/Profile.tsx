...
const UserProfileDropdown = ({ onClose }: UserProfileDropdownProps) => {
  ...
  const menuItems = [
    { icon: (<WalletIcon />), label: "Wallet", count: null, href: "/wallet", event: onClose },
    { icon: (<NotificationIcon />), label: "Notifications", count: 4, href: "#", event: () => { openNotifications(); onClose } },
    { icon: (<CrownIcon />), label: "VIP Club", count: null, href: "/vip-club", event: onClose },
    { icon: (<LikeIcon />), label: "Alliance Plan", count: null, href: "/alliance", event: onClose },
    { icon: (<MedalStarAlt2Icon />), label: "Game records", count: null, href: "/wallet?tab=gamehistory", event: onClose },
    { icon: (<DoughnutChartIcon />), label: "Data Statistics", count: null, href: "/wallet?tab=datastatistics", event: onClose },
    { icon: (<SliderAltIcon />), label: "Settings", count: null , href: "/settings", event: onClose},
    { label: `Default Lang: ${currentLanguage.name}`, count: null, flag: currentLanguage.code, href: "#", event: onClose },
  ];
  ...
  return (
    <div className="lg:w-[282px] md:w-[282px] w-full h-screen lg:h-auto lg:rounded-[14px] glass-bg p-0 text-white font-montserrat backdrop-blur-[32px] overflow-y-auto">
        ...
        <div className="p-4 rounded-xl">
          ...
          <div className="flex glass-input rounded-xl p-1.5 gap-1">
            <input
              ...
              style={{ color: '#55657E' }}
            />
            <FlatButton className="text-xs font-bold h-9 px-4">Send</FlatButton>
          </div>
        </div>
      </div>
              ...
              >
                {
                 ...
                 <span className={`flex items-center gap-2 text-left text-sm font-bold transition-colors ${
      ...
      <div className="px-4 pb-8 lg:pb-2">
        <button onClick={() => setAuthUser(null)} className="flex items-center text-casper gap-2 h-12 px-4 rounded-lg hover:bg-white/5 transition-colors group w-full backdrop-blur-[32px]">
          <DockRightArrowIcon />
